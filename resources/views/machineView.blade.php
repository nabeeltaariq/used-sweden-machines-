...
<div class="links_navigated_pages" style="font-family:arial;font-size:11px;margin-top:0px;">
 ...
 <div class="partsControl" id="partsControl" >
     ...
     <a href="{{URL::to('cart')}}"><i class="glyphicon glyphicon-shopping-cart"></i> Cart  <sup id="totalItems1">{{(Request::session()->has("cartData") ? count(Request::session()->get("cartData")) : '')}}</sup></a>
    ...
    <td style="padding-bottom:4px"><img src="{{URL::to('/storage/app/products/')}}/<?= $part->image ?>" height='120px' width='120px'/></td>
        <td style="padding-left:10px">
            ...
            <b>Delivery Status: </b><b class="text-success"><?= $part->ds ?></b><br/>
              <button onclick="processRequest(this)" data="partNo={{$part->spare_part_no}}&amp;partTitle={{$part->title}}&amp;price={{$part->price}}&amp;status={{$part->ds}}&amp;manu={{($manufacturer ?? '' != null ? $manufacturer->title : '')}}" style="display:inline-block;border:1px solid #ccc;padding:10px;background-color:maroon;color:white"> Add to Basket</button>
        </td>
    </tr>
    @endforeach
